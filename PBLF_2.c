...
struct plane
{
    ...
    int y;
    bool still_live;
    int lives;      //生命次数,wbw_1
    int hit_count;      //撞击次数，wbw_1
...
{ // init graph
    gameInit();
    while (playerPlane.still_live)      //wbw_1
    {
        if(kbhit())
        {
            planeMove();
...
newplane->x=x_random;//we need to make it random        //wbw_1
newplane->y=0;
